...
Manhattan_Plot <- Single_Plot(Data = Intelligence_Sum_Stats, Label_Angle = 60, Verbose = F)
...
- It is often better to produce your own LD matrix using tools such as PLINK1.9/2, the results of which can also be fed in to this package.
```{r, results="hide", warning = FALSE, message = FALSE}
...
```{r, results="hide", warning = FALSE, message = TRUE}
  Regional_Plots <- Regional_Plot(Data = c(Intelligence_Sum_Stats, Household_Income_Sum_Stats),
                                Chromosomes = c(9),  Genome_Build =  c("grch37", "grch37"),
...
for (i in seq_along(Regional_Plots)) {
  inner <- Regional_Plots[[i]]

  for (plot_name in names(inner)) {
    plot <- inner[[plot_name]]

    file_safe <- gsub("[^A-Za-z0-9]", "_", plot_name)
    filename  <- file.path(plots_dir, paste0(file_safe, "_LmultiD.jpg"))
...
Miami_Plot <- Miami_Plot(Top_Data = Sum_Stats[1], Bottom_Data = Sum_Stats[2],
                         ...
                         Label_Angle = 80, Label_Height = c(2,10))
...
Forest_Plot <- Forest_Plot(Data = Intelligence_Sum_Stats, Chromosomes = 4, X_Axis_Text_Resolution = 3)
...
Forest_Plot <- Forest_Plot(Data = Sum_Stats, order = c("P_Label", "BETA_CI_Label"), Right_Size = 12,
                           ...
                           X_Axis_Separation = 0.01, X_Axis_Text_Resolution = 3)
...
Forest_Plot <- Forest_Plot(Data = c(Model_One, Model_Two), order = c("P_Label", "CI_Label"),                                                          Right_Size = 12, Left_Size = 13, Model_Reference = T, 
                                         Axis_Buffer = 0.05, Unprocessed = T,
                Names = c("BMI", "Dementia"), Right_Fill = "lightgreen", Left_Fill = "turquoise" ,
               Strips = F, Styles = c("bold", "underline"), Null_Line_Type = "solid", Verbose = F)
...
```{r,  warning = FALSE, message = FALSE}
  ...
  scroll_box(width = "100%")
```
# Single_With_Regional_Plot() {.tabset}

## Base Example

- Although Single_Plot() and Regional_Plot() are incredibly useful and come with plenty of customisation, often it is more informative to integrate them in to a single compound Manhattan plot. 

- The Single_With_Regional_Plot() function allows users to pass one or more Regional_Plot() objects to a baseline Single_Plot() to be added automatically to their regions of interest.

- Customisation passed to either function is also maintained and legends combined in a useful fashion.

- Here is how we can combine the below exemplar Single and Regional plots.
```{r, results="hide", warning = FALSE, message = FALSE}
plots_dir <- here("vignettes", "Plots")
...
ggplot2::ggsave("Compound_Household_Income_Plot.jpg", plot = Annotated_Man, width = 30,
                 height = 15, units = "in", dpi = 600)
```
Let's inspect our saved compound Manhattan Plot.
```{r,  warning = FALSE, message = FALSE,  fig.width=30, fig.height=15, dpi=300, out.width="100%"}
knitr::include_graphics(paste0(plots_dir,"/Compound_Household_Income_Plot.jpg"))
```



# Segregate_HLA_Plot() {.tabset}

## Base Example

- At times Regional_Plot() can fail to capture the genetics signatures underlying highly complex regions of the genome.

- Such regions include the MHC and HLA gene cluster where additional data beyond SNPs tends to offer better insight for deconvolution of such regions of association.

- HLA-WAS with specialised imputation (e.g. via MEPs) provide association results for the MHC region, stratified by different calls, such as standard SNPs but also amino acid and field resolution allele calls.

- MiamiR also includes a function called Segregate_HLA_Plot() which allows for line plot based, gene stratified visualation of all types of HLA data in addition to passing standard information internally to the Regional_Plot() function, improving analysis efficiency.

- The package also includes a simulated HLA-call based GWAS summary statistics file called "Fake_HLA_10K" which we we'll used and svae outputs of using included helper function: Save_Segregate_HLA_Plots()
```{r, results="hide", warning = FALSE, message = FALSE}
HLA_Plots <- Segregate_HLA_Plot(Data = "Fake_HLA_10K")

setwd(plots_dir)

message("Saving")

Save_Segregate_HLA_Plots(
  res = HLA_Plots,
  dir = "HLA_plots",   
  dpi = 100           
)
...
```
Let's inspect our saved set of plots.
```{r,  warning = FALSE, message = FALSE,  fig.width=30, fig.height=15, dpi=300, out.width="100%"}
img_dir   <- file.path(plots_dir, "HLA_plots")

HLA_img_paths <- list.files(
  path       = img_dir,
  pattern    = "\\.jpg$",     
  recursive  = FALSE,        
  full.names = TRUE
...
- A utility function also included in the MiamiR package is the METASOFT_File_Gen() function which takes multiple datasets as inputs and munges them by default to the METASOFT meta-analysis format.
...
- The MiamiR package also supplies a function called METASOFT_Run() 

- This can take inputs like the above and call METASOFT on them within the R and package environment.

- METASOFT arguments can also be supplied and by default a cleaned output with REGENIE formatted columns is supplied for further, downstream analysis.
```{r, results="hide", warning = FALSE, message = FALSE}
METASOFT_Output <- METASOFT_Run(Data = METASOFT_Example_Input )
```
Let's preview the METASOFT_Run() output.
```{r,  warning = FALSE, message = FALSE}
  METASOFT_Output |>
    dplyr::filter(!is.na(PVALUE_FE)) |>
    dplyr::slice_head(n = 5) |>
    knitr::kable(format = "html", digits = 3, caption = "") |>
