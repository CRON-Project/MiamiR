...
output:
  html_document:
    ...
    toc_float:
      collapsed: true
      smooth_scroll: true
    theme: readable   # try cosmo, cerulean, journal, lumen, paper, sandstone, simplex, spacelab, united, yeti
vignette: >
...
# in setup: library(htmltools); library(ggplot2) if you pass ggplots
carousel <- function(plots,
                     save_dir = knitr::fig_path(),   # where to write images if ggplots
                     file_prefix = "slide",
                     device = "png",
                     dpi = 144,
                     width_in = 8,
...
# img_dir <- plots_dir
# 
# # List files that end with 'Regional_Plot_LmultiD.jpg'
# img_paths <- list.files(
#   path = img_dir,
...
Last, but not least within the MiamiR package main functions is the Tube_Alloys() function. This function combines Single_Plot(), Regional_Plot() and Forest_Plot() functionality in a blunt fashion to output overall plots, regions of interest and SNP and GWAS modelling information. 
...
Other formats may be specified and allele matching is automatically taken care of, similarly to the Forest_Plot() function, with outputs maintaining all SNPs which match the backbone dataset. Here is how to use this simple function using our usual data sets.
```{r, results="hide", warning = FALSE, message = TRUE}
...
na.omit(METASOFT) |>
  ...
  scroll_box(width = "100%")  # keeps table from spanning full page
```
