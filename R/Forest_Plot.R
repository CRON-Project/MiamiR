...
#  numbar <- ceiling(exact_num_underscores)
  ...
  numbar <- floor(exact_num_underscores * (SNP_Stat_Text_Size /2.2)) #21.5
...
#    )
 ...
 # res <- dplyr::bind_rows(res, underline_rows)
  ...
  numbars <- floor(label_length_ratio * (SNP_Stat_Text_Size / 2.65))
...
#        labels <- gsub("(?<=e)f", "</b><span style='font-size:0'>x</span><b>–", labels, perl = TRUE)
        ...
        ifelse(
          ...
          ifelse(
            ...
            paste0("<span style='font-family: Arial; font-size:", SNP_Stat_Text_Size_Bol , "pt; font-weight:bold; color:black'>", formatted_labels, "</span>"),
            paste0("<span style='font-family: Arial; font-size:", SNP_Stat_Text_Size, "pt; color:black'>", formatted_labels, "</span>")
          )
        )
      }
  ...
  ggplot2::theme(
   ...
   family = "Arial", #this bit controls left, above controls right!
      ...
      vjust  = 0.495,  # Adjust vertical alignment to center labels on the tick - bigger = more down 0.642
      hjust = 1
    ),
     axis.text.y.right =  ggtext::element_markdown(
