...
print(string_widths)
 ...
 # underscore_width <- 20
  ...
  numbar <- ceiling(exact_num_underscores * (SNP_Stat_Text_Size / 21.5))
...
#  numbar <- ceiling(exact_num_underscores)
  ...
  numbar <- floor(exact_num_underscores * (SNP_Stat_Text_Size /21.5))
...
#    )
 ...
 # res <- dplyr::bind_rows(res, underline_rows)
  ...
  numbars <- floor(label_length_ratio * (SNP_Stat_Text_Size / 26.85))
  ...
  underline_rows <- res %>%
    ...
    mutate(
      ...
      P_BETA_SE = paste0(Right_Spaces_Two, "\u2009", "\u2009", "zz", vapply(numbar, function(n) paste(rep("\u2501", n), collapse = ""), character(1))),
      Plot_Value = NA,
      Left_Plot_Value = NA,
      Add_Neg = NA,
      Style = NA,
